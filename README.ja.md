...
- **コマンドストリップ** — スラッシュコマンドやフリーテキストフレーズを自動検出し、ターミナル下部にショートカットボタンとして表示。クリックで即再実行、最近使った順にソート。"+" ボタンから手動追加も可能
- **フォント選択** — インストール済みの等幅フォントから選択可能。ライブプレビュー付きで、変更は即座にターミナルに反映
- **動的マウスモード** — 複数ペイン時は tmux マウスモードを自動有効化（ペインごとのスクロールとクリック選択）、単一ペインでは無効化（GhosttyKit ネイティブスクロール）
- **カスタマイズ** — CLI 種別、モデル、effort レベル、承認モード、フォント、動作を設定画面から変更可能
...
- [Sparkle](https://github.com/sparkle-project/Sparkle) — macOS 向け自動アップデートフレームワーク
